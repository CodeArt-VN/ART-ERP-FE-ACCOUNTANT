...
import { BANK_AccountProvider, BANK_IncomingPaymentProvider, BANK_OutgoingPaymentProvider, BANK_TransactionProvider, BRA_BranchProvider, CRM_ContactProvider } from 'src/app/services/static/services.service';
...
export class BankTransactionDetailPage extends PageBase {
  ...
  loadedData(event){
    ...
    this.formGroup.get('Amount').valueChanges.subscribe(value => {
      if (value < 0) {
        this.item.PaymentType = 'OutgoingPayment';
      }
      else   this.item.PaymentType =  'IncomingPayment';
      console.log( this.item.PaymentType);
    });
  ...
  CreatePayment(){
    let total = parseFloat(this.formGroup.controls.PaymentDetails.value.reduce((sum,i)=>{
      return sum += (i.Amount||0);
    },0));
    if(this.formGroup.get('Amount').value<0 && total != Math.abs(this.formGroup.get('Amount').value)){
      this.env.showMessage('Amount of transaction and total amount of payment details not match!', 'danger');
      return;
    }
    if(this.formGroup.controls.Amount.value > 0){
      ...
      let outgoingPayment = {
        ...
        IDOriginalTransaction :this.formGroup.get('Id').value,
        Amount : total,
        Status : 'Paid',
        PostingDate : new Date(),
        DocumentDate : this.formGroup.get('TransactionDate').value,
      ...
      this.outgoingPaymentProvider.save(outgoingPayment).then((savedItem:any)=>{
          if(savedItem){
           this.formGroup.get('ReconciliationStatus').setValue('RecordFound');
           this.formGroup.get('ReconciliationStatus').markAsDirty();
           this.saveChange();
  ...
  async showPurchaseOrderModal() {
    ...
    let amountOrder = -this.formGroup.controls.Amount.value - amountInvoice;
    const modal = await this.modalController.create({
  ...
  async showAPInvoiceModal() {
    ...
    this.SelectedInvoiceList = [...this.paymentDetails];
    let amountOrder = parseFloat(this.formGroup.controls.PaymentDetails.value.filter(d=>d.IDOrder).reduce((sum,i)=>{
      return  sum += (i.Amount || 0);
    },0))
    let amountInvoice = -this.formGroup.controls.Amount.value - amountOrder;
    const modal = await this.modalController.create({
  ...
  async updateField(updatedField: any) {
    const index = this.paymentDetails.findIndex(
      (d) => d.IDInvoice === updatedField.IDInvoice && d.IDOrder == updatedField.IDOrder ,
    );
    if (index != -1) {
