...
export class ARInvoiceSplitModalPage extends PageBase {
  ...
  loadData(event) {
    this.item = {
      IDARInvoice: this.selectedInvoice.Id,
      SplitedARInvoices: [],
      IDBusinessPartner: this.selectedInvoice.IDBusinessPartner,
      IDContact: this.selectedInvoice.IDBusinessPartner,
    };
    ...
    this.ARInvoiceDetailProvider.read({
      IDARInvoice: this.selectedInvoice.Id,
    }).then((result: any) => {
      ...
      if (normalItems != undefined) {
        ...
        let line = this.item.SplitedARInvoices.find((i) => {
          i.InvoiceType == 'normal';
        });
      ...
      if (promoItems != undefined) {
        ...
        let line = this.item.SplitedARInvoices.find((i) => {
          i.InvoiceType == 'claimitem';
        });
        this.item.SplitedARInvoices.splice(this.item.SplitedARInvoices.indexOf(line), 1);

        let line1 = this.item.SplitedARInvoices.find((i) => {
          i.InvoiceType == 'claimdiscount';
        });
