...
export class ChildInvoiceComponent extends PageBase {
    ...
    removeAllSelectedRows() {
        ...
        if (this.selectedItems.length > 0) {
            this.env.showPrompt('Bạn có chắc muốn gở bỏ các hóa đơn này khỏi hóa đơn gộp không? (Thao tác này không thể khôi phục, các hóa đơn gở bỏ sẽ được chuyển về trạng thái đã duyệt)', null,
                'Gỡ bỏ (' + this.selectedItems.length + ') hóa đơn đã chọn').then(_ => {
                    let ids = this.selectedItems.map(m => m.Id);

                    this.arInvoiceProvider.RollbackMergedARInvoice({ Ids: ids })
                        .then((resp: any) => {
                            if (resp == 'empty') {
