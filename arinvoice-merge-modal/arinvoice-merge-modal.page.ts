...
export class ARInvoiceMergeModalPage extends PageBase {
    ...
    loadData(event) {
        this.item = { Ids: [], IDBusinessPartner: null, IDBranch: this.selectedInvoices[0].IDBranch, InvoiceDate: null, IDARInvoice: null };
    ...
    loadedData(event) {
        ...
        this.pageProvider.read({ Take: 20, Skip: 0, Status: 'EInvoiceNew', IsDeleted: false }).then((ar: any) => {
            ...
            ar.data.forEach(a => {
                if (a && this.arInvoiceListSelected.findIndex(d => d.Id == a.Id) == -1) {
                    this.arInvoiceListSelected.push({ Id: a.Id, CustomerName: a.CustomerName, InvoiceNo: a.InvoiceNo, IDBusinessPartner: a.IDBusinessPartner, InvoiceDate: a.InvoiceDate });
                }
            });
    ...
    contactSearch() {
        ...
        this.contactList$ = concat(
            ...
            this.contactListInput$.pipe(
                ...
                switchMap(term => this.contactProvider.search({ Take: 20, Skip: 0, Term: term ? term : this.item.IDBusinessPartner }).pipe(
    ...
    arInvoiceSearch() {
        ...
        this.arInvoiceList$ = concat(
            ...
            this.arInvoiceListInput$.pipe(
                ...
                switchMap(term => this.pageProvider.search({ Take: 20, Skip: 0, Status: 'EInvoiceNew', InvoiceNo: term }).pipe(
                    catchError(() => of([])), // empty list on error
    ...
    mergeARInvoice() {
        ...
        let apiPath = {
            method: "POST",
            url: function () { return ApiSetting.apiDomain("AC/ARInvoice/MergeARInvoice/") }
        };
        ...
        return new Promise((resolve, reject) => {
            if (!this.item.Ids.length || !this.item.IDBusinessPartner) {
                this.env.showTranslateMessage('erp.app.pages.accountant.ar-invoice.message.check-merge-invoice-select-customer', 'warning');
            }
                ...
                this.pageProvider.commonService.connect(apiPath.method, apiPath.url(), this.item).toPromise()
                    .then((savedItem: any) => {
                        ...
                        this.env.showTranslateMessage('erp.app.pages.accountant.ar-invoice.message.can-not-save', 'danger');
                        this.cdr.detectChanges();
                        this.submitAttempt = false;
                        reject(err);
                    });
