...
import { BANK_OutgoingPaymentProvider, BRA_BranchProvider, CRM_ContactProvider, HRM_StaffProvider } from 'src/app/services/static/services.service';
...
export class OutgoingPaymentDetailPage extends PageBase {
	...
	preLoadData(event?: any): void {
		...
		Promise.all([this.env.getStatus('OutgoingPaymentStatus'), this.env.getType('PaymentType'), this.env.getType('OutgoingPaymentReasonType')]).then((values: any) => {
			if (values.length) {
				...
				if (this.formGroup.get(key)) {
					if (key == 'OutgoingPaymentDetails') {
						this.itemFromOtherSource['OutgoingPaymentDetails'].forEach((d) => {
							let fg = this.formGroup.get('OutgoingPaymentDetails')['controls'].find((c) => c.get('DocumentEntry').value == d.DocumentEntry);
							if (fg) {
	...
	changeAmount() {
		...
		if (paymentDetails.controls.length > 0) {
			this.env
				.showPrompt('When changing the amount of money, you need to adjust the orders/invoices!', 'Warning', 'Do you want to delete all invoice details ?', 'Delete', 'No')
				.then((_) => {
