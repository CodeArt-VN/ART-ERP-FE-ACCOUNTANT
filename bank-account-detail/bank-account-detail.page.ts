...
import { BANK_AccountProvider, BRA_BranchProvider, FINANCE_GeneralLedgerProvider, LIST_BankProvider } from 'src/app/services/static/services.service';
...
export class BankAccountDetailPage extends PageBase {
	...
	loadedData(event = null) {
		...
		this.accountProvider.read({ Skip: 0, Take: 5000, ID_ne: this.item.Id, Id: this.formGroup.get('IDParent').value, AllChildren: true }).then((values: any) => {
	...
	segmentChanged(ev: any) {
		...
		if (this.segmentView == 's3') {
			if (
				this.formGroup.get('LoanTenor').value &&
				this.formGroup.get('InterestRate').value &&
				this.formGroup.get('TotalOutstandingLoans').value &&
				this.formGroup.get('DisbursementDate').value
			) {
				let query = { Id: this.formGroup.get('Id').value };
				this.pageProvider.commonService
					.connect('GET', 'BANK/Account/GetCollectionSchedule', query)
					.toPromise()
					.then((rs) => {
						console.log(rs);
