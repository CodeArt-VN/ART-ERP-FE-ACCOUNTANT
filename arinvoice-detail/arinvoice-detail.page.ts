...
export class ARInvoiceDetailPage_ extends PageBase {
  ...
  constructor(
    ...
    this.formGroup = formBuilder.group({
      ...
      ReceiveType: new FormControl({
        value: 'EInvoiceReceiveTypeEmail',
        disabled: false,
      }),
      ...
      TypeCreateInvoice: new FormControl({
        value: 'TypeCreateInvoiceGeneral',
        disabled: false,
      }),
      OriginalInvoiceIdentify: new FormControl({
        value: '',
        disabled: true,
      }),
            ...
            },
            {
              ...
              handler: () => {
                if (this.submitAttempt == false) {
                  ...
                  this.EInvoiceService.CreateEInvoice([this.item.Id])
                    .toPromise()
                    .then((resp: any) => {
  ...
  saveChange() {
    ...
    this.pageProvider.apiPath.postItem.url = function () {
      return ApiSetting.apiDomain('AC/ARInvoice/Add');
    };
    this.pageProvider.apiPath.putItem.url = function (id) {
      return ApiSetting.apiDomain('AC/ARInvoice/Update/') + id;
    };
  ...
  changeTypeCreateInvoice() {
    if (this.formGroup.get('TypeCreateInvoice').value == 'TypeCreateInvoice1') {
      ...
      if (this.item.UserDefine == null || this.item.UserDefine == '') {
        ...
        else this.formGroup.get('UserDefine').clearValidators();
      }
  ...
  loadRelatedARs(pageIndex) {
    ...
    if (
      this.searchRelatedARs_Id != 0 &&
      this.searchRelatedARs_BuyerName != '' &&
      this.searchRelatedARs_Date != '' &&
      this.searchRelatedARs_IdSO != 0
    )
      searchTerm = {
        IsDeleted: false,
        IDParent: this.id,
        Id: this.searchRelatedARs_Id,
        CustomerName: this.searchRelatedARs_BuyerName,
        IDSaleOrder: this.searchRelatedARs_IdSO,
        InvoiceDate: this.searchRelatedARs_Date,
        Skip: skip,
        Take: take,
      };
    else if (
          ...
          buttons: [
            ...
            },
            {
              ...
              handler: () => {
                ...
                this.EInvoiceService.RollbackMergedARInvoice({
                  Ids: ids,
                }).then((resp: any) => {
                  if (resp == 'empty') {
